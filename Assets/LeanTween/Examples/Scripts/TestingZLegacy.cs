...
public class TestingZLegacy : MonoBehaviour
{
    ...
    void cycleThroughExamples()
    {
        if (exampleIter == 0)
        {
            ...
            Time.timeScale = useEstimatedTime ? 0 : 1f; // pause the Time Scale to show the effectiveness of the useEstimatedTime feature (this is very useful with Pause Screens)
            if (timingType == TimingType.HalfTimeScale)
                Time.timeScale = 0.5f;

            if (timingType == TimingType.VariableTimeScale)
            {
                descrTimeScaleChangeId = LeanTween
                    .value(gameObject, 0.01f, 10.0f, 3f)
                    .setOnUpdate(
                        (float val) =>
                        {
        ...
        LeanTween
            .delayedCall(gameObject, delayTime, cycleThroughExamples)
    ...
    public void updateValue3Example()
    {
        ...
        LeanTween
            .value(
                gameObject,
                updateValue3ExampleCallback,
                new Vector3(0.0f, 270.0f, 0.0f),
                new Vector3(30.0f, 270.0f, 180f),
                0.5f
            )
    ...
    public void colorExample()
    {
        ...
        LeanTween
            .color(lChar, new Color(1.0f, 0.0f, 0.0f, 0.5f), 0.5f)
    ...
    {
        Debug.Log(
            "customTweenExample starting pos:" + ltLogo.transform.position + " origin:" + origin
        );

        LeanTween
            .moveX(ltLogo, -10.0f, 0.5f)
            .setEase(customAnimationCurve)
            .setUseEstimatedTime(useEstimatedTime);
        LeanTween
            .moveX(ltLogo, 0.0f, 0.5f)
            .setEase(customAnimationCurve)
            .setDelay(0.5f)
            .setUseEstimatedTime(useEstimatedTime);
    }
    ...
    public void moveExample()
    {
        ...
        LeanTween
            .move(ltLogo, new Vector3(-2f, -1f, 0f), 0.5f)
            .setUseEstimatedTime(useEstimatedTime);
    ...
    public void rotateExample()
    {
        ...
        LeanTween
            .rotate(ltLogo, new Vector3(0f, 360f, 0f), 1f)
            .setEase(LeanTweenType.easeOutQuad)
            .setOnComplete(rotateFinished)
            .setOnCompleteParam(returnParam)
    ...
    public void scaleExample()
    {
        ...
        LeanTween
            .scale(
                ltLogo,
                new Vector3(currentScale.x + 0.2f, currentScale.y + 0.2f, currentScale.z + 0.2f),
                1f
            )
            .setEase(LeanTweenType.easeOutBounce)
            .setUseEstimatedTime(useEstimatedTime);
    ...
    public void updateValueExample()
    {
        ...
        LeanTween
            .value(
                gameObject,
                (Action<float, object>)updateValueExampleCallback,
                ltLogo.transform.eulerAngles.y,
                270f,
                1f
            )
            .setEase(LeanTweenType.easeOutElastic)
            .setOnUpdateParam(pass)
            .setUseEstimatedTime(useEstimatedTime);
    }
    ...
    public void delayedCallExample()
    {
        ...
        LeanTween
            .delayedCall(0.5f, delayedCallExampleCallback)
            .setUseEstimatedTime(useEstimatedTime);
    }
    ...
    public void moveLocalExample()
    {
        ...
        LeanTween
            .moveLocal(lChar, new Vector3(0.0f, 2.0f, 0.0f), 0.5f)
            .setUseEstimatedTime(useEstimatedTime);
        LeanTween
            .moveLocal(lChar, origPos, 0.5f)
            .setDelay(0.5f)
            .setUseEstimatedTime(useEstimatedTime);
    }
    ...
    public void rotateAroundExample()
    {
        ...
        GameObject lChar = GameObject.Find("LCharacter");
        LeanTween
            .rotateAround(lChar, Vector3.up, 360.0f, 1.0f)
            .setUseEstimatedTime(useEstimatedTime);
    }
