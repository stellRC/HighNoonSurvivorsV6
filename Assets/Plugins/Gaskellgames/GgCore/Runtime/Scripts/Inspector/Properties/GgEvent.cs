...
namespace Gaskellgames
{
    ...
    public class GgEvent : GgEventBase
    {
        ...
        public async void Invoke()
        {
            ...
            if (verboseLogs) { GgLogs.Log(logColor, null, LogType.Log, "{0} waiting for {1} seconds.", instanceName, delay); }
            ...
            switch (waitResult)
            {
                case TaskResultType.Timeout:
                    if (verboseLogs) { GgLogs.Log(logColor, null, LogType.Log, "Unity Event '{0}' Timeout.", instanceName); }
                    return;
                    
                case TaskResultType.Cancelled:
                    if (verboseLogs) { GgLogs.Log(logColor, null, LogType.Log, "Unity Event '{0}' Cancelled.", instanceName); }
                    return;
                    
                default:
                case TaskResultType.Complete:
                    if (verboseLogs) { GgLogs.Log(logColor, null, LogType.Log, "Unity Event '{0}' Invoked.", instanceName); }
